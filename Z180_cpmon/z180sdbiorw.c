/*  z180sdrdwr.c
 *  Z180 SD card read/write routines.
 *  Will also be used for BIOS but not tested yet.
 *
 *  SD card code for my DIY Z180 Computer. This
...
unsigned char cmdbuf[6];   /* buffer to build command in */
...
unsigned char *sdcommand(unsigned char *sdcmdp,
    ...
    putspi(0xff);
    putspi(0xff);
    for (sdcbytes = 6; 0 < sdcbytes; sdcbytes--) /* send bytes */
        {
        putspi(*sdcmdp++);
        }
...
int sdread(unsigned char *rdbuf, unsigned char *rdblkno)
    ...
    for (tries = 80; (0 < tries) && ((rbyte = getspi()) != 0xfe); tries--)
        {
        if ((rbyte & 0xe0) == 0x00)
            {
...
int sdwrite(unsigned char *wrbuf, unsigned char *wrblkno)
    ...
    for (tries = 20;
            0 < tries && (((rbyte = getspi()) & 0x11) != 0x01);
            tries--)
        ;
    if (tries == 0)
        {
