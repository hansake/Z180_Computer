; z180upload.s
;
; Uploader for my DIY Z180 computer. This program
; can upload files to memory using Xmodem,
; execute the uploaded program and reboot.
;
; Modified to be assembled with Whitesmiths/COSMIC tools
;
...
.define GETCTM  = 44000     ; loop counter for ~1 sec timeout
...
startupl:
    ...
    ; upload and go
    di
    ld (memptr), bc    ; where in memory to load the uploaded file
    ld (execadr), de   ; address where execution starts
...
upload:
    ld hl, upload_msg
    call print_string
    out (RAMSEL),a     ; select RAM in lower 32KB address range
...
; Serial output and input routines
;
; sends byte in reg A
; affects: none
putc:
...
getcloop:
    ...
    djnz getcoloop      ; outer loop, number of seconds
...
cttest_msg:
    .text "testing serial input timeout, 10 sec between dots\r\n"
    .text "press any key to stop test\r\n", 0
cttestend_msg:
    .text " - timeout test ready\r\n", 0 

; Variables
memptr:     ;pointer to the memory address where to put data
    .word 0
execadr:    ;address where execution starts
    .word 0
